...
export function AppSidebar({ open, apiConfigured, conversations, logo, onClose, onNewChat, onOpenSettings, onSelectConversation, onDeleteConversation }: AppSidebarProps) {
  const EntryIcon = ({ mode }: { mode: Conversation["entries"][number]["mode"] }) => mode === "text" ? <FileText className="size-4" /> : mode === "video" ? <Video className="size-4" /> : <Image className="size-4" />
  return (
    <>
      <aside className={cn("fixed inset-y-0 left-0 z-40 flex w-[272px] -translate-x-full flex-col border-r border-black/10 bg-[#efede7]/90 p-[25px_18px_18px] backdrop-blur-2xl transition-transform duration-200 md:relative md:translate-x-0", open && "translate-x-0")}>
        ...
        <nav className="mt-8 min-h-0 flex-1 px-1" aria-label="最近对话">
          <p className="text-[13px] font-semibold uppercase tracking-[1.3px] text-[#7e7b74]">最近对话</p>
            ...
            <ScrollArea className="mt-3 h-[calc(100dvh-330px)]">
              <div className="space-y-1 pr-2">
                {conversations.map((conversation) => {
                  const latestEntry = conversation.entries.at(-1)
                  return <ContextMenu key={conversation.id}>
                    <ContextMenuTrigger asChild>
                      <button type="button" onClick={() => onSelectConversation(conversation)} className="flex w-full items-center gap-2.5 rounded-[10px] bg-transparent p-2 text-left outline-none transition-colors hover:bg-white/65 focus-visible:ring-2 focus-visible:ring-ring/30 data-[state=open]:bg-white/65">
                        <span className="grid size-[38px] shrink-0 place-items-center overflow-hidden rounded-[8px] bg-white/70 text-[#77736c]">{latestEntry?.mode === "image" && latestEntry.mediaUrl ? <img src={latestEntry.mediaUrl} alt="" className="size-full object-cover" /> : <EntryIcon mode={latestEntry?.mode ?? "text"} />}</span>
                        <span className="flex min-w-0 flex-col gap-0.5">
                          <strong className="truncate text-[14px] font-medium">{conversation.title}</strong>
                          <small className="text-[11px] text-[#85827a]">{relativeTime(conversation.updated)}</small>
                        </span>
                      </button>
                    </ContextMenuTrigger>
                    <ContextMenuContent>
                      <ContextMenuItem onSelect={() => onDeleteConversation(conversation.id)} className="text-[#be462d] focus:bg-[#be462d]/10 focus:text-[#9e402c]">
